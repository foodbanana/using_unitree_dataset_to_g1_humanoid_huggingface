...
DEFAULT_DATASET_NAME = "datasets--unitreerobotics--G1_WBT_Brainco_Make_The_Bed"
...
ANNOTATED_CSV_BASE_DIR = Path(__file__).resolve().parents[3] / "joint_angle_graphs" / "g1_with_brainco_hand" / "csv files"
GRAPH_IMAGE_BASE_DIR = Path(__file__).resolve().parents[3] / "joint_angle_graphs" / "g1_with_brainco_hand" / "graph_images"

# ----------------------------------
# 2) Joint order / group config (loaded from YAML)
# ----------------------------------
JOINT_CONFIG_DIR = Path(__file__).resolve().parents[2] / "config"
HAND_CONFIG_FILES = {
    "brainco": "hand_brainco.yaml",
    "dex3": "hand_dex3.yaml",
    "inspire": "hand_inspire.yaml",
}

DATASET_JOINT_ORDER: list[str] = []
...
def parse_cli_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input_parquet",
        nargs="?",
        help="Optional parquet file path. If omitted, use dataset name + hub path.",
    )
    parser.add_argument(
        "--dataset-name",
        choices=AVAILABLE_DATASET_NAMES,
        default=DEFAULT_DATASET_NAME,
        help="Dataset folder name under the WBT hub root.",
    )
    return parser.parse_args()


def resolve_parquet_input(input_parquet: str | None, dataset_name: str) -> Path:
    """Resolve parquet input path from CLI arg first, then fallback to dataset root."""
    if input_parquet:
...
def _load_yaml_config(config_path: Path) -> dict[str, object]:
    ...
    if not isinstance(config, dict):
        raise ValueError(f"Invalid config: expected a mapping at top level in {config_path}")
    return config


def _resolve_joint_spec(dataset_name: str) -> tuple[list[str], dict[str, list[str]]]:
    if "Brainco" in dataset_name:
        hand_key = "brainco"
    elif "Dex1" in dataset_name or "Dex3" in dataset_name:
        hand_key = "dex3"
    elif "Inspire" in dataset_name:
        hand_key = "inspire"
    else:
        raise ValueError(f"Unsupported dataset name: {dataset_name}")

    base_config = _load_yaml_config(JOINT_CONFIG_DIR / "g1_base.yaml")
    hand_config = _load_yaml_config(JOINT_CONFIG_DIR / HAND_CONFIG_FILES[hand_key])

    joint_order = base_config.get("joint_order", []) + hand_config.get("joint_order", [])
    joint_groups = {**base_config.get("joint_groups", {}), **hand_config.get("joint_groups", {})}

    if not isinstance(joint_order, list) or not isinstance(joint_groups, dict):
        raise ValueError("Invalid joint spec format: missing 'joint_order' or 'joint_groups'")

    return joint_order, joint_groups
...
def _build_annotation_groups(
    ...
    """Build annotation groups, adding locomotion if available in joint order."""
    ...
    locomotion_joints = [
        ...
        for j in joint_groups.get(key, [])
        if j in joint_order
    ]
    if locomotion_joints:
        groups["locomotion"] = locomotion_joints

    return groups


def configure_joint_spec(dataset_name: str) -> None:
    """Update global joint spec for the selected dataset."""
    global DATASET_JOINT_ORDER
    global JOINT_GROUPS
    global ANNOTATION_GROUPS
    global JOINT_TO_GROUP

    joint_order, joint_groups = _resolve_joint_spec(dataset_name)
    DATASET_JOINT_ORDER = joint_order
    JOINT_GROUPS = joint_groups
    ANNOTATION_GROUPS = _build_annotation_groups(joint_groups, joint_order)

    JOINT_TO_GROUP = {}
    for group_name, joints in ANNOTATION_GROUPS.items():
        for joint_name in joints:
            JOINT_TO_GROUP[joint_name] = group_name
...
def main() -> None:
    ...
    if not dataset_name.startswith(PREFIX):
        raise ValueError(f"Dataset name must start with '{PREFIX}': {dataset_name}")

    configure_joint_spec(dataset_name)

    parquet_file_path = resolve_parquet_input(args.input_parquet, dataset_name)
