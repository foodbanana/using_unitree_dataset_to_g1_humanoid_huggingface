"""
data_preprocessing_baseline_lpf.py
────────────────────────────────────────────────────────────────────
목적 : 로봇 텔레오퍼레이션 시계열 데이터에
       ① Butterworth 저역통과 필터(LPF) 적용 후
       ② 선택한 정규화 방법을 적용하여 시각화한다.

처리 순서 :
    raw state → [LPF] → [정규화] → 시각화
...
# ─── [LPF] 필터 하이퍼파라미터 ───────────────────────────────────────────────
LPF_CUTOFF_HZ = 0.3   # 차단 주파수 (Hz). 값이 작을수록 더 강하게 필터링.
                       # 30fps 기준 나이퀴스트 = 15Hz. 권장 범위: 0.1~10 Hz
LPF_ORDER     = 2     # Butterworth 필터 차수. 권장 범위: 2~6
...
# ─── 출력 경로 ────────────────────────────────────────────────────────────────
# 정규화 결과: graphs/normalized_{norm_method}_lpf/...
# 필터 상태:  graphs/filtered_state_lpf/...
FILTERED_GRAPH_ROOT = Path("/home/taeung/g1_datasets_huggingface/graphs/filtered_state_lpf")
...
# ─── [LPF] Butterworth 저역통과 필터 ─────────────────────────────────────────
def apply_lowpass_filter(
    state_matrix: np.ndarray,
    cutoff_hz: float = LPF_CUTOFF_HZ,
    order: int = LPF_ORDER,
    fs: float = FPS,
) -> np.ndarray:
    """
    Butterworth 저역통과 필터를 관절 시계열 전체에 적용한다.
    filtfilt 사용으로 위상 지연 없음.
    """
    nyq = fs / 2.0
    if cutoff_hz >= nyq:
        raise ValueError(
            f"차단 주파수({cutoff_hz} Hz)가 나이퀴스트 주파수({nyq} Hz) 이상입니다."
        )
    b, a = butter(order, cutoff_hz / nyq, btype="low")
    return filtfilt(b, a, state_matrix, axis=0)
...
def build_output_dir(
    ...
    """정규화 결과 그래프 저장 경로: graphs/normalized_{norm_method}_lpf/..."""
    return _GRAPH_BASE / f"normalized_{norm_method}_lpf" / control_type / hand_type / dataset_name


def build_filtered_output_dir(
    control_type: str, hand_type: str, dataset_name: str
) -> Path:
    """필터 적용 후 관절각 그래프 저장 경로: graphs/filtered_state_lpf/..."""
...
def plot_normalized_episode(
    ...
    fig.suptitle(
        f"[LPF + {norm_method}] Episode {episode_index}"
        f"  |  cutoff={cutoff_hz}Hz  order={lpf_order}",
        fontsize=11,
    )
...
def plot_filtered_state_episode(
    ...
    """LPF 적용 후 관절각 시계열을 그룹별 서브플롯으로 시각화한다."""
    episode_df = episode_df.sort_values("frame_index")
    frame_seconds = episode_df["frame_index"].to_numpy(dtype=float) / FPS
    ...
    axes[-1].set_xlabel("time (s)")
    fig.suptitle(
        f"Filtered Joint State (LPF) — Episode {episode_index}"
        f"  |  cutoff={cutoff_hz}Hz  order={lpf_order}",
        fontsize=11,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.95])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cutoff-hz", type=float, default=LPF_CUTOFF_HZ, metavar="HZ",
        help=(
            f"LPF 차단 주파수 (Hz).\n"
            f"  default: {LPF_CUTOFF_HZ}  (나이퀴스트={FPS / 2:.0f}Hz 미만이어야 함)"
        ),
    )
    parser.add_argument(
        "--lpf-order", type=int, default=LPF_ORDER, metavar="ORDER",
        help=f"Butterworth 필터 차수.\n  default: {LPF_ORDER}",
    )
    parser.add_argument(
        "--norm-method",
    ...
    parser.add_argument(
        ...
        metavar="BOOL",
        help=(
            "True 일 때 LPF 적용 후 데이터에 대해\n"
            "  1초 구간 STD 로그·시각화를 에피소드별로 실행한다.\n"
            "  저장 경로: graphs/1sec_std_lpf/\n"
            "  default: False"
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    print(f"[LPF]  차단 주파수   : {args.cutoff_hz} Hz")
    print(f"[LPF]  필터 차수     : {args.lpf_order}")
    print("=" * 60)
    ...
    # ── [보조] LPF 적용 후 1초 구간 STD 시각화 (--show_1sec_std True 일 때만) ──
    if args.show_1sec_std:
        std1sec_output_dir = (
            _GRAPH_BASE / "1sec_std_lpf" / control_type / hand_type / args.dataset_name
        )
        print(f"\n[보조] 1초 구간 STD 시각화 중  →  {std1sec_output_dir}")
        print(f"       (LPF cutoff={args.cutoff_hz}Hz 적용 후 데이터 기준)\n")
        ...
        for ep_idx in range(args.max_episodes):
            ...
            state_filtered = apply_lowpass_filter(state_matrix,
                                                  cutoff_hz=args.cutoff_hz,
                                                  order=args.lpf_order)
            print(f"  episode {ep_idx:>3} : {len(ep_df)} frames")
            log_and_plot_1sec_std(
                ...
                title_prefix=f"[LPF cutoff={args.cutoff_hz}Hz] ",
            )
    ...
    if args.finding_std is not None:
        ...
        finding_output_dir = (
            _GRAPH_BASE / "finding_std_lpf" / control_type / hand_type / args.dataset_name
        )
        ...
        for ep_idx in range(args.max_episodes):
            ...
            plot_finding_std_episode(
                ...
                filter_desc=f"[LPF cutoff={args.cutoff_hz}Hz order={args.lpf_order}]",
            )
    ...
    if args.show_1sec_std:
        print(f"  1초 구간 STD  : {_GRAPH_BASE / '1sec_std_lpf' / control_type / hand_type / args.dataset_name}")
    if args.finding_std is not None:
        print(f"  finding_std   : {_GRAPH_BASE / 'finding_std_lpf' / control_type / hand_type / args.dataset_name}")
