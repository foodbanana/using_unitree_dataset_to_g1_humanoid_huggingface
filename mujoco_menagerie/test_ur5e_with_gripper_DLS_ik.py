...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        dynamic_step = step_size if dist > 0.05 else 0.01 # 가까워질수록 더 작은 스텝으로 세밀하게 조정
        ...
        if dist > tol:
            ...
            # 4. Damped Least Squares (DLS) 역행렬 계산
            # lambda_sq 값이 클수록 부드러워지지만 느려지고, 작을수록 빨라지지만 떱니다.
            lambda_sq = 0.001  # DLS damping factor
            I = np.eye(3) # 3x3 단위 행렬

            J_arm_T = J_arm.T
            # 공식: J^T * (J * J^T + lambda^2 * I)^-1
            J_inv = J_arm_T @ np.linalg.inv(J_arm @ J_arm_T + lambda_sq * I)
            ...
            for i, act_id in enumerate(actuator_ids):
                data.ctrl[act_id] += dynamic_step * delta_q[i]
